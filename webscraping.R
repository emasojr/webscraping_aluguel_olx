...
for (j in 1:(num_pag)){
  ...
  for (k in 12:ifelse(j<num_pag,61,rf)) {
    ...
    quartos = ifelse(categoria[14]=='',categoria[13],categoria[14])
    categoria = ifelse(categoria[14]=='',categoria[[11]],categoria[[12]])
    ...
    link = divs[[k]]
    datap = as.vector(cas %>% html_nodes('span') %>% html_text())[11]
